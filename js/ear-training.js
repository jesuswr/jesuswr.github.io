...
let selectedIntervals = ['P4', 'P5']; // Default
...
const earState = {
  ...
  locked: false, 
  startTime: null,
  activeIntervals: [] // Will store the full objects for the current session
};

// 2. Piano Sound Engine (FM Synthesis)
function playPiano(freq, startTime, duration) {
  const ctx = getCtx();
  const out = ctx.createGain();
  
  // Basic Piano-like Envelope
  const osc = ctx.createOscillator();
  const modulator = ctx.createOscillator();
  const modGain = ctx.createGain();

  osc.type = 'sine'; // Fundamental
  modulator.type = 'sine'; // Harmonics
  
  // Piano "tine" strike - Modulator freq is often a multiple of fundamental
  ...
  out.gain.linearRampToValueAtTime(0.5, startTime + 0.01); // Quick attack
  out.gain.exponentialRampToValueAtTime(0.001, startTime + duration); // Long decay
...
// 3. Updated Ear Training Play Logic
    ...
    btn.onclick = () => {
      if (selectedIntervals.includes(int.id)) {
        if (selectedIntervals.length > 2) {
          ...
          alert("Select at least 2 intervals");
        }
      } else {
...
// Update the Ear Start Button Listener
document.getElementById('ear-start-btn').addEventListener('click', () => {
  earState.length = parseInt(document.getElementById('ear-length-input').value, 10) || 10;
  earState.activeIntervals = ALL_INTERVALS.filter(i => selectedIntervals.includes(i.id));
  
  // Build sequence from only selected intervals
  const seq = [], freqs = [];
  for (let i = 0; i < earState.length; i++) {
    const randomInt = earState.activeIntervals[Math.floor(Math.random() * earState.activeIntervals.length)];
    seq.push(randomInt.id);
    freqs.push(110 * Math.pow(2, Math.random() * 1.5)); // Random base between A2 and E3
  }
  ...
  renderIntervalButtons(); // Now uses activeIntervals
  updateEarProgress();
  showView('ear-playing-view');
  playCurrentInterval();
});

// Update renderIntervalButtons to use earState.activeIntervals
